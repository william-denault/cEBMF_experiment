...
load("C:/Document/Serieux/Travail/Data_analysis_and_papers/cEBMF_RCC_experiments/additional_experiment_for_rebutall/CMF_spatial_transcriptomics_results/run_spatial_DLPFC_10.RData")
...
p6 <- plot_memberships_on_slice(m,loc,title = "CMF + clustering",
                                colors = c(factor_colors7  ))



ggsave( plot_grid(p1,p4,p6,p2,p3, nrow = 1,ncol = 5),
        file= paste0("C:/Document/Serieux/Travail/Data_analysis_and_papers/cEBMF_RCC_experiments/additional_experiment_for_rebutall/CMF_spatial_transcriptomics_results/plot_slice_",i,".pdf"),
        width =29.7  ,
        height =  6,
        units = "cm"
)
...
image(res$matrices$A)


tdf=  data.frame(loading= res$matrices$A[1, ], x=loc[,1], y=loc[,2])

p11<-  ggplot(tdf, aes(x=x, y=y, color=loading)) +
  geom_point(size=0.6) +
  scale_color_gradient(low="blue", high="red")+  ggtitle("CMF component 1")+
  theme(legend.position = "none")



tdf=  data.frame(loading= cEBMF.obj$loading[,7], x=loc[,1], y=loc[,2])
...
tdf=  data.frame(loading= res$matrices$A[2, ], x=loc[,1], y=loc[,2])

p21<-  ggplot(tdf, aes(x=x, y=y, color=loading)) +
  geom_point(size=0.6) +
  scale_color_gradient(low="blue", high="red")+ggtitle("CMF component 2")+
  theme(legend.position = "none")



tdf=  data.frame(loading= cEBMF.obj$loading[,2], x=loc[,1], y=loc[,2])
...
tdf=  data.frame(loading= res$matrices$A[3, ], x=loc[,1], y=loc[,2])

p31<-  ggplot(tdf, aes(x=x, y=y, color=loading)) +
  geom_point(size=0.6) +
  scale_color_gradient(low="blue", high="red")+ ggtitle("CMF component 3")+
  theme(legend.position = "none")



tdf=  data.frame(loading= cEBMF.obj$loading[,3], x=loc[,1], y=loc[,2])
...
tdf=  data.frame(loading= res$matrices$A[4, ], x=loc[,1], y=loc[,2])

p41<-  ggplot(tdf, aes(x=x, y=y, color=loading)) +
  geom_point(size=0.6) +
  scale_color_gradient(low="blue", high="red")+ ggtitle("CMF component 4")+
  theme(legend.position = "none")



tdf=  data.frame(loading= cEBMF.obj$loading[,4], x=loc[,1], y=loc[,2])
...
ggsave( plot_grid(p11,p12,
                  ...
                  p41,p42                  ,
                  nrow = 4,ncol = 2),
        file= paste0("C:/Document/Serieux/Travail/Data_analysis_and_papers/cEBMF_RCC_experiments/additional_experiment_for_rebutall/CMF_spatial_transcriptomics_results/comp_CMF_cEBMF.pdf"),
        width =21  ,
        height =  29.7,
        units = "cm"
)
