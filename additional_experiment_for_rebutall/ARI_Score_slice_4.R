...
res_cebnm <- Fclust.compare(truth, cL)

res_cebnm

RI.F( truth,loadings[,-7]/colSums(loadings[,-7]),t_norm="product")
res_cebnm <- Fclust.compare( truth,loadings /colSums(loadings ))

spec_cluster <- kmeans(loadings,centers = clusterNum[i], nstart = 100)
clusterlabel= as.numeric(walktrap_clustering(clusternum=clusterNum[i],t(loadings),knearest=70 ))
...
res_cebnm <-Fclust.compare(truth, m)
res_cebnm

RI.F( truth,loadings/colSums(loadings),t_norm="product")
...
res_spaPCA <- Fclust.compare(truth, m)
res_spaPCA
...
res_CMF <- Fclust.compare(truth, t(res$matrices$A))
res_CMF
...
res_MFAI <- Fclust.compare(truth, m)
res_MFAI
