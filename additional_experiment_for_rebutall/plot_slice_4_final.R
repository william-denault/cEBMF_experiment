...
load("C:/Document/Serieux/Travail/Data_analysis_and_papers/cEBMF_RCC_experiments/additional_experiment_for_rebutall/CMF_spatial_transcriptomics_results/run_spatial_DLPFC_4.RData")
loadings_CMF <-  t(res$matrices$A )
norms <- lapply(1:ncol(loadings), function(j) {sum(abs(loadings_CMF[,j]))})
...
cL_CMF <- t(res$matrices$A )
for (j in 1:ncol(cL_CMF)) {
  cL_CMF[, j] <- cL_CMF[, j] / norms[j]
}
...
cL_CMF<- cL_CMF %*%diag(norms)

#spec_cluster <- kmeans(t(res$matrices$A ),centers = clusterNum[i], nstart = 100)
#clusterlabel= as.numeric(walktrap_clustering(clusternum=clusterNum[i], (res$matrices$A),knearest=70 ))
#clusterlabel_refine =as.numeric( refine_cluster_10x(clusterlabels=clusterlabel,
#                                                   location=LIBD@location,shape="hexagon"))
...
p6 <- plot_memberships_on_slice(cL_CMF,loc,title = "CMF ",
                                colors = c(factor_colors7  ))
...
p7 <- plot_memberships_on_slice(m,loc,title = "MFAI + clustering",
                                colors = c(factor_colors7  ))

ggsave( plot_grid(p1,p2,p6,p7,p4, nrow = 1,ncol = 5),
        file= paste0("C:/Document/Serieux/Travail/Data_analysis_and_papers/cEBMF_RCC_experiments/additional_experiment_for_rebutall/plots/plot_slice_",i,".pdf"),
        width =29.7  ,
        height =  6,
        units = "cm"
)
